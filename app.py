...
async def q4(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if answer in [
        ...
        await update.message.reply_text(
            "❗️ Обратите внимание, важно чтобы свидетельство об окончании полицеальной школы было выдано до 30.06.2025 г, а податься на карту резидента нужно успеть до 30.06.2026 г.\n"
            "Если не успеваете по этим датам, то придётся сдать экзамен.\n\n"
...
async def q4_policealna(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if answer == "Хочу узнать про экзамен":
        await update.message.reply_text(
            "Быстрее и эффективнее всего будет сдать государственный экзамен. Вот ссылка на официальный сайт госэкзамена: https://certyfikatpolski.pl/ 🇵🇱\n\n"
            "Если хочешь подготовиться к экзамену, можешь пройти курсы в одной из аккредитованных школ, например: https://szkolapolskiego.pl/\n\n"
            "❗️ Обрати внимание: если твой сертификат об окончании полицеальной школы выдан после 30.06.2025, он не будет приниматься для карты резидента ЕС. В этом случае нужен только государственный экзамен.",
            reply_markup=ReplyKeyboardMarkup([
                ["Понятно, идём дальше"]
            ], one_time_keyboard=True, resize_keyboard=True)
        )
